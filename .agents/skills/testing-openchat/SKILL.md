...
- Every completed/failed run persists the full `request.model_dump()` in sqlite at `<data-dir>/openchat_history.db`, table `conversations.request_json` — even for upstream-401 failures saved as `failed_fusion`. Query it to prove what the client sent (e.g. `json_extract(request_json,'$.service_tiers')`).
