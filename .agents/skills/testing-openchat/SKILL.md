...
- Orchestrated runs persist the full `request.model_dump()` in sqlite at `<data-dir>/openchat_history.db`, table `conversations.request_json` — even for upstream-401 failures saved as `failed_fusion`. Direct chat saves a reconstructed request payload on success only; direct-chat failures and fusion regenerations persist nothing. Query `request_json` to prove what the client sent (e.g. `json_extract(request_json,'$.service_tiers')`).
